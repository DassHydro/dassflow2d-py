...
class TestDischarge1(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.discharge = Discharge1(self.config, boundaries, 1, 'discharg1', 2)


        self.small_node = Node(1.0, 0.5, 0.8)
        self.medium_node = Node(10.0, 5.0, 8.0)
        self.large_node = Node(100.0, 50.0, 80.0)
        self.small_ghost_node = Node(1.234, 0.567, 0.891)
        self.medium_ghost_node = Node(12.34, 5.67, 8.91)
        self.large_ghost_node = Node(123.4, 56.7, 89.1)
    ...
    def test_read_hydrograph(self):
        ...
        self.assertEqual(expected_hydrograph, hydrograph)

    def test_interpolate_dynamic_value(self):

        # Test interpolation at t=18000.0 (should be 0.1555663)
        q = self.discharge.interpolate_dynamic_value(18000.0)
        self.assertAlmostEqual(q, 0.1555663)

        # Test interpolation at t=36000.0 (should be 2.999316)
        q = self.discharge.interpolate_dynamic_value(36000.0)
        self.assertAlmostEqual(q, 2.999316)

        # Test interpolation at t=50000.0 (between 46800.0 and 50400.0)
        q = self.discharge.interpolate_dynamic_value(50000.0)
        expected_q = 3.372389 - (3.372389 - 3.003062) * (50000.0 - 46800.0) / (50400.0 - 46800.0)
        self.assertAlmostEqual(q, expected_q, places=6)

    def test_update_distributes_q_in(self):
        ...
        # Call update
        self.discharge.update({}, self.state, 36000.0)

        # Calculate expected inflows using actual water depths
        expected_inflow_small = -2.999316 * (h_small ** (2/3)) / sum_pow_h
        expected_inflow_medium = -2.999316 * (h_medium ** (2/3)) / sum_pow_h
        expected_inflow_large = -2.999316 * (h_large ** (2/3)) / sum_pow_h

        # Check that the inflow is distributed correctly
        self.assertAlmostEqual(self.small_node.u, expected_inflow_small, places=6)
        self.assertAlmostEqual(self.medium_node.u, expected_inflow_medium, places=6)
        self.assertAlmostEqual(self.large_node.u, expected_inflow_large, places=6)
